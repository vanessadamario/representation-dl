...
class SignalGenerator1D():
    ...
    def generate_signal(self, sigma_ampl, sigma_supp, sparsity, noise=0., balanced=True):
        ...
        # self.pattern_length_array = np.random.uniform(int(length_patterns*0.8),
        #     length_patterns, size=self.classes)
        # self.pattern_amplitude_array = np.random.uniform(1, 5*self.classes,
        #     size=self.classes)
        ...
        for idx_f, f in enumerate(self.function_list):
            ...
            for j in range(samples_per_class):
                tmp_x_in_signal = np.random.choice(all_x_in_signal,
                    replace=False, size=n_patterns_per_signal)
...
def main():
    ...
    sg.generate_signal(0.2, 0.2, 0)
    X = sg.X

    # plt.figure(figsize=(5, 25))
    plt.subplots(nrows=8, ncols=1, sharex=True)

    idx_class = np.arange(0, 1000, 125)
